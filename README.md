...
As we worked on this project, we learned more about features of JavaFX, specifically customizing buttons and tables and incorporating images. We faced challenges while implmenting some of the objectives which caused us to either modify them or research/try ways we can work around the issue (we acknowledge that some solutions are not optimal but sufficient enough for proof of concept). If we were to continue working on this project, we would modify our program to be able to parse through larger datasets and improve the runtime. Additionally, we would consider ways of styling the layout to provide a better visual representation of our app. Overall we are satisfied with what we produced within the limited amount of time we had and hope that users can find our application useful.
